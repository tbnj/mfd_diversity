...
### Import data
meta <- data.table::fread("output/2025-02-19_MFD_metadata_BC_subset.tsv") %>%
  mutate(across(complex, ~factor(.))) %>%
  mutate(across(complex, ~droplevels(.)))

dist <- data.table::fread("output/2025-03-18_MFD_BC_distance_subset.csv") %>%
...
### Run PERMANOVA
set.seed(123)
permanova.total <- adonis2(dist ~ complex, data = meta, by = "terms", permutations = 999, parallel = 10)

df.permanova.total3 <- data.frame(permanova.total) %>%
...
data.table::fwrite(df.permanova.total, sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE,
                   paste0("output/", format(Sys.time(), "%Y-%m-%d"), "_MFD_PERMANOVA.tsv"))
...
for (i in 1:length(levels.test)){
  ...
  set.seed(123)
  permanova<- vegan::adonis2(dist ~ test,
...
data.table::fwrite(df.permanova.contrasts, sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE,
                   paste0("output/", format(Sys.time(), "%Y-%m-%d"), "_MFD_PERMANOVA_contrasts.tsv"))

### Save image
save.image(paste0('output/', format(Sys.time(), "%Y-%m-%d"), "_MFD_PERMANOVA.RData"))
