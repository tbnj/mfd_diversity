...
### Import data
## Load metadata
metadata <- data.table::fread("data/2025-04-15_MFD_FL16S_metadata_subset.tsv", sep = "\t", header = T) %>%
...
fl.asv.subset.ra <- data.table::fread("data/2025-04-15_MFD_FL16S_OTU_filtered_ra.csv", sep = ",", header = TRUE)
...
data.table::fwrite(dt, sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE,
                   paste0("output/", format(Sys.time(), "%Y-%m-%d"), "_MFD_FL16S_alpha_diveristy_analysis.csv"))
...
plot.alpha <- fl.alpha.diversity %>%
  ...
  geom_text(data = dt, aes(x = complex, y = 250, label = cld),
...
data.table::fwrite(fl.alpha.diversity, sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE,
                   paste0("output/", format(Sys.time(), "%Y-%m-%d"), "_MFD_FL16S_alpha_diversity_MFDO1.tsv"))


### Gamma
## Load gamma table
data.gamma.ra <- data.table::fread("data/2025-04-15_MFD_FL16S_iNEXT_richness_MFDO1_ra.tsv", sep = "\t", header = TRUE)
# data.gamma <- data.table::fread("data/2025-02-25_MFD_FL_iNEXT_richness_MFDO1.tsv", sep = "\t", header = TRUE)
...
plot.gamma.total <- fl.gamma.diversity %>%
  ...
  scale_fill_manual(values = mfdo1.palette.subset,
                    name = "MFDO1") +
  facet_grid(cols = vars(Hill_diversity), scales = "free_y") +
  coord_flip() +
  theme_bw(base_size = 12) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
...
plot.gamma.shannon <- fl.gamma.diversity %>%
  ...
  geom_errorbar(aes(ymin = LCL, ymax = UCL), width = 0.5, position=position_dodge(width=0.9)) +
  # geom_point(shape = 21) +
  geom_text(aes(y = 35000, label = str_c("N = ", samples)), size = 4,
            fontface = "bold") +
  scale_x_discrete(limits = plot.order) +
  scale_fill_manual(values = mfdo1.palette.subset,
                    name = "MFDO1") +
...
data.table::fwrite(fl.gamma.diversity, sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE,
                   paste0("output/", format(Sys.time(), "%Y-%m-%d"), "_MFD_FL16S_gamma_diversity_MFDO1.tsv"))
...
data.table::fwrite(table.diversity, sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE,
                   paste0("output/", format(Sys.time(), "%Y-%m-%d"), "_MFD_FL16S_combined_diversity_MFDO1.tsv"))


### Save image
save.image(paste0('output/', format(Sys.time(), "%Y-%m-%d"), "_MFD_FL16S_diversity.RData"))
